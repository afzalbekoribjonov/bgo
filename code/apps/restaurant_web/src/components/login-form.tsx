...
import { isRestaurant, requestOtp, setToken, verifyOtp } from '@/lib/auth';
  ...
  async function verify() {
    ...
    try {
      ...
      if (!isRestaurant(user)) {
        setError("Sizda oshxona (restaurant) huquqi yo'q");
        setLoading(false);
        return;
      }
      ...
      <div className="card">
        ...
        {step === 'phone' ? (
          <>
            ...
            <button className="btn" style={{ marginTop: 12 }} disabled={loading} onClick={send}>
          ...
          <>
            ...
            {error && <p className="error">{error}</p>}
            <button className="btn" style={{ marginTop: 12 }} disabled={loading} onClick={verify}>
              {loading ? '...' : 'Kirish'}
            </button>
            <button
              className="btn ghost"
