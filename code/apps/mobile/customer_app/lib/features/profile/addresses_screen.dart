...
class AddressesScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    // O'chirish — qaytarib bo'lmas amal, avval tasdiq so'raymiz.
    Future<void> confirmDelete(Address a) async {
      final ok = await showDialog<bool>(
        context: context,
        builder: (ctx) => AlertDialog(
          title: Text(t.addressDelete),
          ...
          actions: [
            ...
            FilledButton(
              ...
              child: Text(t.addressDelete),
            ),
          ],
        ),
      );
      if (ok == true) {
        await run(() => ref.read(profileApiProvider).deleteAddress(a.id));
      }
    }
        ...
        data: (items) {
          ...
          return ListView.separated(
            ...
            itemBuilder: (_, i) => _AddressCard(
              ...
              onDelete: () => confirmDelete(items[i]),
            ),
          );
        },
      ),
          ...
          children: [
            Container(
              ...
              child: Icon(Icons.add_location_alt_outlined,
                  size: 42, color: scheme.primary.withValues(alpha: 0.7)),
            ),
            ...
            Text(
              'Uy va ish manzillaringizni saqlang —\nbuyurtma berish tezlashadi',
  ...
  Widget build(BuildContext context) {
    ...
    return Card(
      shape: RoundedRectangleBorder(
        ...
        // Standart manzil — brend chegarasi bilan ajralib turadi.
        side: address.isDefault
            ? BorderSide(
                color: scheme.primary.withValues(alpha: 0.45), width: 1.4)
            : BorderSide.none,
      ),
      child: Padding(
