...
/**
 * Rasm yuklash (admin yoki sotuvchi). ImageKit sozlangan bo'lsa — unga,
 * aks holda lokal diskka saqlanadi va gateway orqali statik ko'rsatiladi.
 */
@Controller('marketplace/upload')
  ...
  async upload(@UploadedFile() file?: UploadedImageFile) {
    ...
    if (this.imagekit.isConfigured) {
      const url = await this.imagekit.uploadBuffer(
        file!.buffer,
        `marketplace-${Date.now()}${ext}`,
        '/beshariq-marketplace',
      );
    ...
    return {
      success: true,
      data: { url: `${base}/api/v1/marketplace/uploads/${name}` },
    };
  }
