...
import { UPLOADS_DIR } from './marketplace/upload.controller';
...
async function bootstrap() {
  ...
  app.useStaticAssets(UPLOADS_DIR, {
    prefix: '/api/v1/marketplace/uploads/',
    maxAge: '30d',
    immutable: true,
  });
  ...
  const port = process.env.MARKETPLACE_PORT ?? 4006;
  await app.listen(port);

  Logger.log(`🏪 Marketplace servisi: http://localhost:${port}/api/v1`, 'Bootstrap');
